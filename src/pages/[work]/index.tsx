...
import convertToTitleCase from "@/lib/services/fs/get-work-title";
import getFileInJson from "@/lib/services/fs/get-file-json";
import getWorksIndex from "@/lib/services/fs/get-work-index";
...
type WorkDynamicPageProps = {
  title: string;
  data: Source;
};

const Work: NextPage<WorkDynamicPageProps> = ({ title, data }) => {
  const { content, title: workTitle, kind } = data || {};
  console.log("data", data);
  return (
    <div className="flex flex-col w-full text-justify md:pt-0">
      ...
      {content && (
        <>
          {Array.from(content).map((c, i) => (
            <Fragment key={i}>
              {kind && kind === "sonnets" && (
                <p className="text-center p">{i + 1}</p>
              )}
              <p className="p-2">{c}</p>
            </Fragment>
          ))}
...
export const getStaticProps: GetServerSideProps = async ({ params }) => {
  ...
  // const content = await getFileInJson(work_id).catch((err) => {
  //   throw new Error(
  //     `Error while reading 'src/text/sources/${work_id}' : ${err}`
  //   );
  // });

  return {
    props: {
      title: convertToTitleCase(work_id),
      data: content,
    },
  };
};
